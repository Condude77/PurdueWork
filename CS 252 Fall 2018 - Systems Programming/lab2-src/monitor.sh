...
function generate_report {
	...
	if [ $CURRENT_REPORTS -eq $MAXIMUM_REPORTS ] ; then
		#Get the oldest file in report directory
		OLD_FILE=`ls -1t ./reports_dir | tail -1`
		
		#Delete the old file
		rm ./reports_dir/$OLD_FILE
		
		#Decrease total number of reports
		let CURRENT_REPORTS=CURRENT_REPORTS-1
	fi
	...
	#Increase total number of reports
	let CURRENT_REPORTS=CURRENT_REPORTS+1
}
...
function calculate_mem_usage
{
	...
	VM=$( egrep VmRSS /proc/$PID/status )
	
	#Set mem_usage to the proper value
	mem_usage=`echo $VM | awk '{ print $2 }'`
...
function notify
{
	...
	if [ $cpu_usage_int -gt $CPU_THRESHOLD ] || [ $2 -gt $MEM_THRESHOLD ] ; then
		NEW_REPORT=`ls -1t ./reports_dir | head -1`
		mailx -s "Usage Report" $USER < ./reports_dir/$NEW_REPORT
	fi
