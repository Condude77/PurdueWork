...
public class CFGTest {
    ...
    @Test
    //added unreachable created node test to make sure that created nodes with no edge should not be reachable
    public void unreachableNode(){
	//add unconnected nodes	
	cfg.addNode(9998, m_m, m);
	cfg.addNode(9999, m_m, m);
	//check that they are not reachable
	assertFalse(cfg.isReachable(9999, m_m, m, 9998, m_m, m));
    }

    @Test
    //added addEdge duplicate test to to make sure that it checks for if duplicate edges are handled correctly
	...
	CFG.Node n = new CFG.Node(1000, m_m, m);
	int nodeSize = cfg.edges.get(n).size();
	
	//add same nodes and edges
	cfg.addNode(1000, m_m, m);
	cfg.addEdge(1000, m_m, m, 1001, m_m, m);

	//check that nothing was broke by duplicating edges
	assertTrue(nodeSize == cfg.edges.get(n).size());
    }
